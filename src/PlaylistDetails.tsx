...
function PlaylistPopularity({tracks}: any){
  const totalPopularity = tracks.reduce((acc: any, curr: any) => acc + curr.track.popularity, 0)

  return (<>
    <p className='text-2xl font-semibold'>Average Song Popularity: {(totalPopularity / tracks.length).toFixed(0)}</p>
    <p className="text-xs text-justify">The popularity of the track. The value will be between 0 and 100, with 100 being the most popular.
...
function PlaylistDetails({playlistId}: any) {
  ...
  const { data, isLoading, isError } = useQuery('playlist_details_' + playlistId, () => getPlaylistDetails(token, playlistId));
